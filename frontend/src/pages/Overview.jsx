...
export default function Overview({ data, alerts = [], onNavigate }) {
  ...
  const weekSales = days.reduce(
    (total, day) => total + Number(day.sales || 0),
    0,
  );
    ...
    <>
      <section className="metric-grid">
        <MetricCard
          icon={icons.chart}
          label="Sales today"
          value={formatMoney(today.sales || 0)}
          note={`${today.transactions || 0} transactions`}
        />
        <MetricCard
          icon={icons.arrowUp}
          label="Profit today"
          value={formatMoney(today.profit || 0)}
          note="From today's sales"
          positive
        />
        <MetricCard
          icon={icons.items}
          label="Retail items left"
          value={Number(inventory.repackRemaining || 0).toLocaleString()}
          note={`${inventory.productCount || 0} products in stock`}
        />
        <MetricCard
          icon={icons.bell}
          label="Needs attention"
          value={inventory.lowStockCount || 0}
          note={`${alerts.filter((a) => !a.resolved).length || inventory.unresolvedAlertCount || 0} unresolved alerts`}
          warning
        />
      </section>
        ...
        <article className="card sales-card">
          <div className="card-heading">
            <div>
              ...
              <p>Daily revenue over the last 7 days</p>
            </div>
          ...
          <div className="chart-bars">
            {days.map((day) => (
              <div className="bar-column" key={day.date}>
                <span className="bar-value">{formatMoney(day.sales)}</span>
                <div className="bar-track">
                  <span
                    style={{
                      height: `${Math.max(8, (Number(day.sales || 0) / maxSales) * 100)}%`,
                    }}
                  />
                </div>
        ...
        <article className="card stock-card">
          <div className="card-heading">
            <div>
              ...
              <p>Restock these products soon.</p>
            </div>
            <button
          ...
          {lowStock.length ? (
            <div className="stock-list">
              {lowStock.map((product) => (
                <div className="stock-row" key={product.productId}>
                  ...
                  <span className="stock-count">
                    {product.repackRemaining} left
                  </span>
                </div>
              ))}
            </div>
          ) : (
            <Empty label="Your stock levels look good." />
          )}
        </article>
          ...
          {recentSales.length ? (
            <div className="sales-table">
              ...
              {recentSales.map((sale) => (
                <div className="table-row" key={sale._id}>
                  <span>
                    <strong>
                      {sale.productId?.name ||
                        sale.productName ||
            ...
            <Empty label="Your recorded sales will appear here." />
          )}
        </article>
      </section>
    </>
